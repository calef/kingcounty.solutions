...
- `mayhem new` – Creates a new Jekyll site with kingcounty.solutions configuration.
...
**Purpose**
Creates a new Jekyll site with kingcounty.solutions-specific configuration, including AP style settings, collections, content age limits, and required plugins.
...
- Jekyll must be installed (via `bundle exec jekyll` or directly).
- Git must be installed.

**Behavior notes**

- Validates Jekyll and Git are installed before proceeding.
- Creates a new Jekyll site using `jekyll new` if the target directory doesn't exist or lacks `_config.yml`.
- Initializes a Git repository if the directory isn't already a Git repo.
- Updates `_config.yml` with kingcounty.solutions configuration:
  - `ap_style` – AP style date/time formatting rules
  - `collections` – Collections for events, images, organizations, locations, and topics
  - `content_max_age_days: 365` – Maximum age for content
  - `defaults` – Layout defaults for each collection type
  - `future: true` – Show future-dated posts
  - `rss_max_item_age_days: 90` – RSS feed age limit
  - `timezone: America/Los_Angeles` – Pacific timezone
- Adds required Jekyll plugins to Gemfile and runs `bundle install`:
  - jekyll-feed
