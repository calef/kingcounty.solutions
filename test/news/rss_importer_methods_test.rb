...
class RssImporterMethodsTest < Minitest::Test
  ...
  def test_unchanged_post_detects_matching_checksum
    normalized_html = Mayhem::Support::HtmlNormalizer.normalize('<p>body</p>', base_url: 'https://source')
  ...
  def write_post(filename, front_matter, body = '')
    ...
    File.write(path, Mayhem::Support::FrontMatterDocument.build_markdown(front_matter.merge('original_content' => front_matter['original_content'] || ''), body))
    path
  end
