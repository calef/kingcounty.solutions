...
class SourceUrlCheckerTest < Minitest::Test
  ...
  def test_not_found_url_unpublishes_post
    post_id = write_post('2025-01-01-test.md', 'https://example.com/missing')
  ...
  def test_not_found_url_clears_post_images
    ...
    post_id = write_post_with_image_checksums('2025-01-01-test.md', 'https://example.com/missing', [image_id])
  ...
  def test_not_found_url_removes_unreferenced_images
    ...
    assert_nil Mayhem::Models::Image.find_by(checksum: unique_image), 'Image metadata should be removed'
  ...
  def test_not_found_url_preserves_shared_images
    ...
    refute_nil Mayhem::Models::Image.find_by(checksum: shared_image), 'Shared image metadata should remain'
  end
  ...
  def test_not_found_event_is_deleted
    event_id = write_event('event123', 'https://example.com/missing-event')

    checker = build_checker(http_client: ->(_url) { :not_found })
    checker.run

    assert_raises(FMRepo::NotFound, 'Event should be deleted') { Mayhem::Models::Event.find(event_id) }
  end

  def test_successful_event_check_does_not_delete_event
    event_id = write_event('event456', 'https://example.com/valid-event')

    checker = build_checker(http_client: ->(_url) { :success })
    checker.run

    refute_nil Mayhem::Models::Event.find(event_id), 'Event should not be deleted'
  end

  def test_deleted_event_removed_from_post_references
    event_id = write_event('event789', 'https://example.com/missing-event')
    post_id = write_post_with_event_ids('2025-01-01-test.md', 'https://example.com/valid', [event_id])
  ...
  def test_event_without_source_url_is_not_checked
    event_id = write_event_without_source_url('event123')

    checker = build_checker(http_client: ->(_url) { :not_found })
    checker.run

    refute_nil Mayhem::Models::Event.find(event_id), 'Event without source_url should not be deleted'
  end
  ...
  def test_multiple_valid_posts_and_events_remain_unchanged
    post_id = write_post('2025-01-01-valid.md', 'https://example.com/valid')
    event_id = write_event('event1', 'https://example.com/event-valid')
  ...
  def test_multiple_invalid_posts_and_events_are_cleaned_up
    post_id = write_post('2025-01-02-missing.md', 'https://example.com/missing')
    event_id = write_event('event2', 'https://example.com/event-missing')
  ...
  def test_multiple_event_references_cleaned_from_single_post
    event1_id = write_event('event1', 'https://example.com/missing1')
    event2_id = write_event('event2', 'https://example.com/missing2')
    event3_id = write_event('event3', 'https://example.com/valid')
    post_id = write_post_with_event_ids(
      '2025-01-01-test.md',
  ...
  def test_post_with_only_deleted_events_has_empty_events_array
    event_id = write_event('event1', 'https://example.com/missing')
    post_id = write_post_with_event_ids(
      '2025-01-01-test.md',
      'https://example.com/valid',
      [event_id]
