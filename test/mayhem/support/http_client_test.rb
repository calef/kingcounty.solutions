...
class HttpClientTest < Minitest::Test
  ...
  def test_resolve_final_url_returns_successful_redirect
    @client.instance_variable_get(:@request_flow).stub(:resolve_head_redirects, { status: 200, url: 'https://final' }) do
  ...
  def test_resolve_final_url_returns_nil_for_non_successful_status
    @client.instance_variable_get(:@request_flow).stub(:resolve_head_redirects, { status: 400, url: 'https://start' }) do
  ...
  def test_response_for_returns_hash_with_status_and_url
    ...
    @client.instance_variable_get(:@request_flow).stub(:fetch_with_redirects, [response.new('200'), payload]) do
  ...
  def test_perform_http_head_uses_net_http_connection
    ...
    Net::HTTP.stub(:new, ->(_host, _port) { fake_http }) do
      result = @client.instance_variable_get(:@transport).send(:perform_http_head, URI('https://example.com'), OpenSSL::SSL::VERIFY_PEER)
  ...
  def test_follow_head_redirect_follows_multiple_hops
    ...
    transport.stub(:execute_head, proc { |_uri, **_| responses.shift }) do
      result = @client.instance_variable_get(:@request_flow).send(
  ...
  def test_too_many_requests_delay_respects_parse_result_and_minimum
    ...
    response_processor = @client.instance_variable_get(:@response_processor)
    response_processor.stub(:parse_retry_after_value, 5) do
      assert_equal 5, response_processor.send(:parse_retry_after, response)
    end
    response_processor.stub(:parse_retry_after_value, 0) do
      assert_equal 60, response_processor.send(:parse_retry_after, response)
    end
  end

  def test_parse_retry_after_handles_numeric_httpdate_and_invalid
    response_processor = @client.instance_variable_get(:@response_processor)
    assert_equal 5, response_processor.send(:parse_retry_after_value, '5')

    now = Time.now
    Time.stub(:now, now) do
      header = (now + 5).httpdate
      assert_equal 5, response_processor.send(:parse_retry_after_value, header)
    end

    assert_nil response_processor.send(:parse_retry_after_value, 'not-a-date')
  end

  def test_perform_request_follows_redirect
    response = FakeResponse.new('301', { 'location' => 'https://example.com/next' }, redirect: true)
    transport = @client.instance_variable_get(:@transport)
    transport.stub(:execute_get, response) do
      redirected = false
      ...
      request_flow.stub(:follow_redirect, proc { |*_| redirected = true; :redirected }) do
        result = request_flow.send(:perform_request, 'https://example.com', 'text/html', 2, 2, origin_url: 'https://example.com', operation: 'op')
        assert redirected
  ...
  def test_perform_request_raises_on_too_many_requests
    ...
    transport.stub(:execute_get, response) do
      assert_raises(Mayhem::Support::HttpClient::TooManyRequestsError) do
        @client.instance_variable_get(:@request_flow).send(:perform_request, 'https://example.com', 'text/html', 2, 2, origin_url: 'https://example.com', operation: 'op')
      end
    end
  end
  ...
  def test_execute_request_retries_without_verification
    ...
    transport.stub(:perform_http_request, proc { |_uri, _accept, _verify_mode| raise error }) do
      transport.stub(:retry_without_verification, proc { called = true; :retry }) do
        assert_equal :retry, transport.send(:execute_get, URI('https://example.com'), 'text/html')
        assert called
      end
    end
  end
  ...
  def test_execute_head_request_retries_without_verification_head
    ...
    transport.stub(:perform_http_head, proc { |_uri, _verify_mode| raise error }) do
      transport.stub(:retry_without_verification_head, proc { called = true; :rehead }) do
        assert_equal :rehead, transport.send(:execute_head, URI('https://example.com'))
        assert called
      end
    end
  end
  ...
  def test_follow_head_redirect_handles_too_many_requests_and_missing_location
    ...
    transport = @client.instance_variable_get(:@transport)
    transport.stub(:execute_head, proc { |_uri, **_| error_response }) do
      assert_raises(Mayhem::Support::HttpClient::TooManyRequestsError) do
        @client.instance_variable_get(:@request_flow).send(:follow_head_redirect, URI('https://example.com'), 1, origin_url: 'origin', operation: 'op')
      end
    end

    redirect_response = FakeResponse.new('301', {}, redirect: true)
    transport.stub(:execute_head, proc { |_uri, **_| redirect_response }) do
      assert_equal({ url: 'https://example.com', status: 301 }, @client.instance_variable_get(:@request_flow).send(:follow_head_redirect, URI('https://example.com'), 1, origin_url: 'origin', operation: 'op'))
    end
  end
  ...
  def test_follow_head_redirect_respects_remaining_redirects
    response = FakeResponse.new('301', { 'location' => 'https://example.com/next' }, redirect: true)
    transport = @client.instance_variable_get(:@transport)
    transport.stub(:execute_head, proc { |_uri, **_| response }) do
      assert_raises(RuntimeError) do
        @client.instance_variable_get(:@request_flow).send(:follow_head_redirect, URI('https://example.com'), 0, origin_url: 'origin', operation: 'op')
      end
    end
  end
  ...
  def test_retry_without_verification_logs_and_retries_only_when_allowed
    ...
    denial_transport = denial_client.instance_variable_get(:@transport)
